...
(function($){
    jQuery.fn.radioSwitch = function(options, switched_callback) {
    	...
    	var settings = {
            ...
            first_color               : '#febdbd',
            first_color_shadow        : '#dca3a3',
            second_color              : '#c8df77',
            second_color_shadow       : '#adc267',
    		// Handle
    		handle_img                : '/images/slider.png',
    		handle_bg_color           : '#999',
    		handle_border_color       : '#d0d0d0',
    		// Labels
    		label_text_color          : "#222",
    		label_font_size           : 12
    	};
    	...
    	return this.each(function() {
            ...
            var last_label    = container.find('label:last-child');
    		...
    		var left   = ((settings.width+settings.label_width/2) * state) + settings.padding;
    		var handle_display = 'block'//(state == -1 ? 'none' : 'block');
    		...
    		container.css({
                         ...
                         'box-shadow':'rgb(153, 153, 153) 2px 2px 5px inset',
	                     });
    		/**** make the track ****/
    		track = jQuery('<div />')
    		...
    		labels.css({
    		           ...
    		           'user-select':'none',
    		           'z-index':'2',
                       'margin-right':(settings.width-settings.label_width)*2,
    		           })
    		...
    		container.find('label').click(function() {
    		    ...
    		    // Positions
    		    var position_left    = ((settings.width + settings.label_width/2) * state);
    		    ...
    		    // Values
                myContainer.find('input').removeAttr('checked');
    		    myRadio.attr('checked', true)
    		           .trigger('change');
    		    myHandle.css({display:display_handle});
                if (myContainer.find('input[checked=checked]').attr('name')==container.find('input:first-child').attr('name')) {
                    myContainer.css({
                        'background-color':settings.second_color,
                        'box-shadow': 'inset 1px 1px 2px '+settings.second_color_shadow,
                    });
                    myContainer.parent().addClass('yes');
                } else {
                    myContainer.css({
                        'background-color':settings.first_color,
                        'box-shadow': 'inset 1px 1px 2px '+settings.first_color_shadow,
                    });
                    myContainer.parent().removeClass('yes');
                }
    		    // Animate handle
